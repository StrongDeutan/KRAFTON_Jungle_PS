...
def solve(locations: list, start: int, end: int):
    ...
    sub = []                            # 경계선의 왼쪽으로 ans // 2 오른쪽으로 ans // 2내의 x좌표 점들

    for i in range(mid):
        if(locations[mid - i][0] < border_line - ans // 2):
            break
        sub.append(locations[mid - i])
        if(locations[mid + i][0] > border_line + ans // 2):
            break
        sub.append(locations[mid + i])
    ...
    sub.sort(key=cmp_to_key(compare_by_y))
...
locations.sort(key=cmp_to_key(compare_by_x))
